...
def test_manifest_matches_current_blocks_and_form_order() -> None:
    ...
    assert manifests["prompt-preset"].subagent_overrideable is False
    assert manifests["prompt-preset"].worker_overrideable is True
    assert manifests["prompt-preset"].subagent_policy == "force-remove"
    assert manifests["worker-delegation"].worker_policy == "guarded"
    assert manifests["worker-delegation"].tool_names == ("run_worker",)
...
def test_editor_defaults_are_derived_from_current_authoring_contracts() -> None:
    ...
    worker_delegation = defaults["worker_delegation"]
...
def test_editor_text_snapshots_match_locked_upstream_defaults() -> None:
    ...
    assert not hasattr(subagents, "TASK_SYSTEM_PROMPT")
