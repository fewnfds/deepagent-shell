...
RUNTIME_FOLDER = "automation_plugins"
...
def read_plugin_requirements(folder: Path) -> PluginRequirements:
    path = folder / REQUIREMENTS_FILE
    if not path.exists():
        values: tuple[str, ...] = ()
        return PluginRequirements(values, _requirements_fingerprint(values))
    if not path.is_file() or _is_link(path):
        raise ResourceScanError(
            "resource.error.automationScript.requirementsLinkUnsupported",
    ...
    try:
        ...
        raise ResourceScanError(
            "resource.error.automationScript.requirementsReadFailed",
            "requirements.txt could not be read.",
        ) from exc
    if len(content) > MAX_REQUIREMENTS_BYTES:
        raise ResourceScanError(
            "resource.error.automationScript.requirementsTooLarge",
    ...
    try:
        ...
        raise ResourceScanError(
            "resource.error.automationScript.requirementsInvalidEncoding",
    ...
    for line_number, raw_line in enumerate(text.splitlines(), start=1):
        ...
        if name in parsed:
            raise ResourceScanError(
                "resource.error.automationScript.requirementsDuplicate",
        ...
        if len(parsed) > MAX_REQUIREMENTS_COUNT:
            raise ResourceScanError(
                "resource.error.automationScript.requirementsTooMany",
...
def plugin_runtime_root(runtime_root: Path) -> Path:
    return runtime_root / RUNTIME_FOLDER


def dependency_state_path(runtime_root: Path) -> Path:
    return plugin_runtime_root(runtime_root) / STATE_FILE


def plugin_site_packages(runtime_root: Path) -> Path:
    return plugin_runtime_root(runtime_root) / SITE_PACKAGES_FOLDER
...
def dependency_metadata(
    ...
    if not requirements.values:
        ...
        plugin_state = (
            state.get("plugins", {}).get(script_id)
            if state is not None
            else None
        )
        if (
            isinstance(plugin_state, dict)
            and plugin_state.get("requirements_fingerprint")
            == requirements.fingerprint
            and plugin_state.get("status") in {"ready", "failed"}
        ):
            status = plugin_state["status"]
            error_code = str(plugin_state.get("error_code", ""))
        else:
...
def activate_plugin_site(runtime_root: Path) -> None:
    state = load_dependency_state(runtime_root)
    site_packages = plugin_site_packages(runtime_root)
    ...
    if site_text not in sys.path:
        # Core site-packages stays first so plugins cannot shadow locked core modules.
...
def _input_fingerprint(
    ...
    payload = {
        ...
        "plugins": [
            ...
            for item in sorted(plugins, key=lambda value: str(value["id"]))
    ...
    *,
    ...
    _atomic_write_json(
        dependency_state_path(runtime_root),
        {
            ...
            "plugins": _plugin_states(
                plugins, status="failed", error_code=error_code
            ),
        },
    )
...
def _ensure_uv(runtime_root: Path, manifest: dict[str, Any]) -> Path:
    ...
    temporary_root = runtime_root / "tmp" / f"uv-plugin-{uuid4().hex}"
...
def _replace_site_packages(runtime_root: Path, prepared: Path) -> None:
    target = plugin_site_packages(runtime_root)
...
def _plugin_layout_is_supported(site_packages: Path) -> bool:
...
def prepare_windows_dependencies(
    *,
    ...
    catalog = scan_automation_scripts(
        data_root / "resources" / "automation_scripts",
        runtime_root=None,
    )["catalog"]
    plugins = [dict(item) for item in catalog]
    input_fingerprint = _input_fingerprint(core_fingerprint, plugins)
    ...
    if (
        ...
        print("Automation plugin dependencies are already current.")
        return

    requirements = [
        requirement
        for plugin in plugins
        for requirement in plugin["python_requirements"]
    ]
    build_root = runtime_root / "tmp" / f"automation-plugins-{uuid4().hex}"
    ...
    try:
        if requirements:
            ...
            if result.returncode != 0:
                ...
                print(
                    "WARNING: Automation plugin dependencies could not be resolved. "
                    "Agent Shell will start without the plugin dependency layer.",
                    file=sys.stderr,
                )
                return
            if not _plugin_layout_is_supported(prepared):
                _write_failure_state(
                    runtime_root,
                    plugins,
                    input_fingerprint=input_fingerprint,
                ...
                print(
                    "WARNING: Automation plugin dependencies contain unsupported "
        ...
        _atomic_write_json(
            dependency_state_path(runtime_root),
            {
                ...
                "input_fingerprint": input_fingerprint,
                "core_fingerprint": core_fingerprint,
                "plugins": _plugin_states(plugins, status="ready"),
            },
        )
        print("Automation plugin dependencies are ready.")
    finally:
        shutil.rmtree(build_root, ignore_errors=True)


def main(arguments: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="python -m agent_shell.automation.dependencies")
    ...
    try:
        ...
        print(f"Automation plugin dependency preparation failed: {exc}", file=sys.stderr)
        return 1
    return 0
