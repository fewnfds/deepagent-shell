    ...
    const session: AgentSessionTimeline = {
      ...
      runs: [
        run('one', [
          ...
          { step_id: 'event-5', sequence: 6, kind: 'tool_result', timestamp: null, data: { tool_name: 'read_file' } },
          { step_id: 'event-6', sequence: 7, kind: 'model_request', timestamp: null, data: { message_count: 0 } },
          { step_id: 'event-7', sequence: 8, kind: 'subagent', timestamp: null, data: { subagent_name: 'Researcher' } },
        ]),
        run('two', [
