export const zhCN = {
  ...
  errors: {
    ...
    pythonPackageFolderImmutable: '已保存配置不能更换私有包文件夹。',
    pythonPackageComponentUnsupported: '该组件类型不支持私有 Python 包。',
    pythonPackageInvalid: '私有 Python 包无效。',
    pythonPackageReadFailed: '无法读取私有 Python 包；请根据错误代码和请求 ID 检查包文件与 Debug 日志。',
    pythonPackageNotFound: '私有 Python 包不存在或不属于当前配置。',
    pythonPackageTemplateInvalid: '所选 Python 代码模板无效。',
    pythonPackageTemplateRevisionConflict: '代码模板已变化，请重新载入模板后再保存。',
    pythonPackageRevisionConflict: '私有 Python 包已变化，请重新载入后再保存。',
  ...
  validation: {
    ...
    resolution: {
      ...
      pythonPackageFolderFormatInvalid: '请重新载入配置，以恢复其私有包文件夹引用。',
      ...
      pythonPackageConfigInvalid: '修正不符合包配置 Schema 的字段。',
    ...
    issue: {
      ...
      contract: {
        ...
        pythonPackageFolderFormatInvalid: '{field}必须是当前配置拥有的私有包文件夹。',
      ...
      pythonPackage: {
        ...
        configInvalid: 'Python 包 {package_id} 的配置无效（{keyword}）。',
      },
      storage: {
