...
const profiles = ref<WorkerProfileRecord[]>([])
const selectedProfileId = ref('')
const form = ref(blankWorkerProfile())
let profileLoadSequence = 0

const feedbackIsError = computed(() => (
  feedbackKey.value.endsWith('Failed') || feedbackKey.value === 'agents.serviceUnavailable'
))
const overrideableManifests = computed(() => manifests.value.filter((manifest) => manifest.worker_overrideable))

const { markClean, runAfterDiscard } = useUnsavedChanges(
  () => workerProfilePayload(form.value),
  () => ({
...
const { validation, validateNow } = useDraftValidation(
  form,
  () => ({
    target: { kind: 'worker-profile', id: form.value.id },
    payload: workerProfilePayload(form.value),
  }),
  async (request) => {
...
const displayedValidation = computed(() => validation.value.status === 'unavailable'
  ? {
    ...validation.value,
    error: managementError.describe(validation.value.error, 'errors.validationUnavailable').display,
  }
  : validation.value)
...
function selectionValue(type: CapabilityType): string {
  const selection = workerOverrideSelection(form.value, type)
  if (selection.mode === 'inherit') return INHERIT_VALUE
  if (selection.mode === 'disabled') return DISABLED_VALUE
  return selection.block_id
}

function updateSelection(type: CapabilityType, value: string): void {
  if (value === INHERIT_VALUE) {
    setWorkerOverrideSelection(form.value, type, 'inherit')
  } else if (value === DISABLED_VALUE) {
    setWorkerOverrideSelection(form.value, type, 'disabled')
  } else {
    setWorkerOverrideSelection(form.value, type, 'replace', value)
  }
}
...
async function loadProfile(id: string): Promise<void> {
  ...
  feedbackDetail.value = ''
  try {
    const loaded = normalizeWorkerProfile(await service.value.getWorkerProfile(id))
...
async function loadSelected(value?: string): Promise<void> {
  await runAfterDiscard(() => loadProfile(value ?? selectedProfileId.value))
}

function upsertProfile(saved: WorkerProfileRecord): void {
...
async function save(): Promise<void> {
  ...
  try {
    await validateNow()
    const payload = workerProfilePayload(form.value)
    const saved = form.value.id
      ? await service.value.updateWorkerProfile(form.value.id, payload)
      : await service.value.createWorkerProfile(payload)
    const normalized = normalizeWorkerProfile(saved)
...
async function loadWorkspace(): Promise<void> {
  ...
  try {
    ...
    profiles.value = profileItems.map(normalizeWorkerProfile)
...
watch(() => route.query.id, (value) => {
  ...
  if (id !== selectedProfileId.value) void loadProfile(id)
})
</script>
  ...
  <PageShell>
    ...
    <div class="row g-3 align-items-start">
      <section class="col-lg-8">
        ...
        <section class="card mb-3">
          <header class="card-header">
            <h2 class="card-title">{{ t('agents.worker.contextTitle') }}</h2>
          </header>
          <div class="card-body">
            <div class="form-check form-switch">
              <input id="include-client-messages" v-model="form.include_client_messages" class="form-check-input" type="checkbox">
              <label class="form-check-label" for="include-client-messages">
                {{ t('agents.worker.includeClientMessages') }}
              </label>
              <div class="form-text">{{ t('agents.worker.includeClientMessagesHint') }}</div>
            </div>
          </div>
        </section>

        <section :aria-label="t('agents.worker.capabilitiesTitle')">
          <div class="row g-3">
            <div v-for="capability in overrideableManifests" :key="capability.type" class="col-md-6 col-xxl-4">
              <section class="card h-100">
                <header class="card-header">
                  <label class="card-title mb-0" :for="`worker-capability-${capability.type}`">
                    {{ t(`capabilities.${capability.type}.label`) }}
                  </label>
                </header>
                <div class="card-body">
                  <select
                    :id="`worker-capability-${capability.type}`"
                    class="form-select"
                    :value="selectionValue(capability.type)"
                    @change="updateSelection(capability.type, ($event.target as HTMLSelectElement).value)"
                  >
                    <option :value="INHERIT_VALUE">{{ t('agents.worker.mode.inherit') }}</option>
                    <option v-if="!capability.required" :value="DISABLED_VALUE">{{ t('agents.worker.mode.disabled') }}</option>
      ...
      <aside class="col-lg-4">
        <ValidationChecklist :title="t('agents.worker.validationTitle')" :validation="displayedValidation" />
      </aside>
    </div>
  </PageShell>
</template>
