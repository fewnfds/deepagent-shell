...
- 组件 YAML 只保存扩展代码目录引用和满足 Schema 的普通 `config`。
...
当前编辑器只直接维护 `main.py` 和 `requirements.txt`。配置扩展中的额外模块、vendor 目录和其他文件保持原样，用户可在目录中
...
配置扩展是受信任的任意代码，以 Agent Shell 服务进程权限运行，不是 sandbox。不要在模板、配置扩展、manifest 或普通 config 中
