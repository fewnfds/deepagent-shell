...
Agent Shell 通过管理台组合 Deep Agents 配置，并以固定 Graph（图）和 Entry Script（入口脚本）提供 OpenAI-compatible model。

推荐顺序：

1. [启动并认识管理台](getting-started.md)
2. [创建组件](capabilities.md)
3. [创建 Graph 工作流](graph-workflows.md)
4. [装配 Main Agent 与 Subagent](configuration-workflow.md)
5. [使用自动化插件](automation.md)
6. [管理配置仓库](configuration-library.md)
7. [调用 API Server](api-server.md)
8. [查看最终 ModelRequest](interception-test.md)
9. [查看日志与历史会话](runtime-observability.md)
