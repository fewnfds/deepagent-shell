...
Command 节点是 Workflow 画布上可同时更新 State 和动态选择后继目标的可编程节点。组件配置保存一个独占的 Python 扩展目录引用和管理台显示的有序文件路径；
...
新建页可以选择现有模板，也可以【套用空模板】从 `main.py` 和空的 `requirements.txt` 开始。用户可以逐行增加包内相对文件路径；编辑器按清单顺序显示并保存这些文本文件。
